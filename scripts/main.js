...
function setupCiteImages() {
    ...
    cites.forEach(cite => {
        cite.addEventListener('click', (e) => {
            ...
            // CSS vars set inline on the cite span. On mobile the img is
            // appended to <body> and no longer inherits, so copy them.
            const propagateVars = [
                '--img-brightness',
                '--img-bottom-m', '--img-width-m', '--img-height-m', '--img-left-m'
            ];
            propagateVars.forEach(name => {
                const value = cite.style.getPropertyValue(name);
                if (value) img.style.setProperty(name, value);
            });
